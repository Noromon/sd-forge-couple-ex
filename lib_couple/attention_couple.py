...
from .attention_masks import get_mask, lcm_for_list, sharpen_mask
...
class AttentionCouple:
    ...
    def patch_unet(
        ...
        if separation_mode == "Latent":
            # ===== Latent mode: independent k/v per region =====
            # Instead of concatenating all conds into one k/v, we run
            # separate attention per region and mask-blend the outputs.
            @torch.inference_mode()
            def attn2_patch(q, k, v, extra_options):
                ...
                for i, cond_or_uncond in enumerate(cond_or_unconds):
                    ...
                    if cond_or_uncond == 1:  # uncond
                        ...
                        # Repeat q for each cond, but keep k separate per cond
                        # We interleave: q repeated num_conds times, k = [k_target, cond_0, cond_1, ...]
                        qs.append(q_chunks[i].repeat(num_conds, 1, 1))
                        ks.append(torch.cat([k_target] + cond_ks, dim=0))
            ...
            def attn2_output_patch(out, extra_options):
                ...
                # In Latent mode, the output layout per cond chunk is:
                # [batch_size (original k), batch_size (cond_0), batch_size (cond_1), ...]
                # We only want the cond outputs, skip the first batch_size (original k)
                outputs = []
                pos = 0
                for cond_or_uncond in cond_or_unconds:
                    if cond_or_uncond == 1:  # uncond
                        ...
                        # Skip original k output, take cond outputs
                        pos += cls.batch_size  # skip original k
                        chunk = out[pos : pos + num_conds * cls.batch_size]
                        chunk = chunk.view(num_conds, cls.batch_size, out.shape[1], out.shape[2])

                        # mask_downsample is [num_conds * batch_size, tokens, 1]
                        # reshape to [num_conds, batch_size, tokens, 1]
                        mask_reshaped = mask_downsample.view(
                            num_conds, cls.batch_size, out.shape[1], 1
                        )
                        masked_output = (chunk * mask_reshaped).sum(dim=0)
            ...
            def attn2_output_patch(out, extra_options):
                ...
                # Sharpen mask: reshape to [num_conds, batch_size, num_tokens, 1]
                # so that dim 0 is the cond dimension for sharpen_mask
